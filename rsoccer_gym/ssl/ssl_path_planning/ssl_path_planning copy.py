...
class SSLPathPlanningMultiEnv(SSLBaseEnv):
    ...
    def reward_function(self, robot, last_robot, target_pos, target_angle, target_vel):
        max_dist = np.sqrt(self.field.length ** 2 + self.field.width ** 2)
        robot_pos = Point2D(robot.pos.x, robot.pos.y)
        robot_vel = Point2D(robot.v_x, robot.v_y)
        robot_angle = np.deg2rad(robot.theta)
        robot_angular_vel = np.deg2rad(robot.v_theta)
        last_robot_pos = Point2D(last_robot.pos.x, last_robot.pos.y)
        ...
        if dist_robot_to_target <= DIST_TOLERANCE:
            ...
            if robot_velocity_to_target <= SPEED_TOLERANCE and angular_velocity_to_target <= ANGULAR_SPEED_TOLERANCE:
                if angle_error <= ANGLE_TOLERANCE:
                    return angle_reward + 3, True, info
                else:
                    return angle_reward, False, info

            return angle_reward + velocity_reward, False, info

        return dist_reward + angle_reward, False, info
    ...
    def _calculate_reward_and_done(self):
        ...
        for i in range(self.n_robots):
            ...
            _rew, _done, _info = self.reward_function(robot, last_robot, target_pos, target_angle, target_vel)
            # TODO: IF DONE, RESET TARGET AND INFOS
            self.reward_info[i].update(_info)
            reward.append(_rew)
            dones.append(_done)
        return reward, done
    ...
    def _get_initial_positions_frame(self):
        ...
        for i in range(self.n_robots):
            ...
            while places.get_nearest(pos)[1] < min_gen_dist:
                pos = (get_random_x(), get_random_y())
            self.target_point[i] = Point2D(x=pos[0], y=pos[1])
